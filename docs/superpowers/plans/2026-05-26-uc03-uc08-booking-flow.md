...
- [x] **Step 1: Implement `releaseExpiredBookings` in `BookingsService`**
...
- [x] **Step 1: Update `TripsService` to support search**
...
- [x] **Step 2: Update `TripsController` with `GET /trips/search`**
  - Add `@Get('search')` handler taking query parameters `origin`, `destination`, and `departureDate`.

- [x] **Step 3: Write tests for UC03 in `trips.service.spec.ts` and `trips.controller.spec.ts`**
...
- [x] **Step 1: Update `SeatsService` with `findOrCreateSeatsForTrip`**
...
- [x] **Step 2: Update `SeatsService` with `lockSeat`**
...
- [x] **Step 3: Update `SeatsController`**
  - Add `GET /seats/trip/:tripId` to return the grid.
  - Add `POST /seats/lock` (Protected by `FirebaseAuthGuard`).

- [x] **Step 4: Write tests for UC04 in `seats.service.spec.ts` and `seats.controller.spec.ts`**

---

### Task 4: Implement UC05 & UC06 - Đặt vé & Thanh toán VietQR

- [x] **Step 1: Update `BookingsService.create` to handle booking requests**
  - Call the expiration cleanup helper.
...
- [x] **Step 2: Update `BookingsController`**
  - Add `POST /bookings` (Protected by `FirebaseAuthGuard`).

- [x] **Step 3: Update `PaymentsService` and `PaymentsController`**
...
- [x] **Step 4: Write tests for UC05 and UC06 in `bookings.service.spec.ts` and `payments.service.spec.ts`**

---

### Task 5: Implement UC07 - Lịch sử đặt vé

- [x] **Step 1: Implement `myHistory` in `BookingsService`**
  - Call the expiration cleanup helper.
...
- [x] **Step 1: Implement `cancelBooking` in `BookingsService`**
...
- [x] **Step 2: Run linter and verify zero errors**
- [x] **Step 3: Rebuild and check Docker logs to ensure perfect deployment**
